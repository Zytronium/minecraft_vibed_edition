// State module - contains the State struct and all game logic
use std::sync::Arc;
use std::sync::mpsc;
use winit::{
    window::Window,
...
use crate::{GameMode, PlayerState, ChunkMesh, build_texture_map};
use crate::camera::Camera;
use crate::vertex::Vertex;
use crate::world::{World, CHUNK_WIDTH, CHUNK_HEIGHT};
use crate::ui;
use crate::block;
use crate::models;
use crate::menu::WorldSize;
use crate::save;
...
pub struct State {
    ...
    pub chunk_meshes: Vec<ChunkMesh>,  // Separate buffers for each chunk
    pub world: World,  // Keep world for block modifications
    pub camera: Camera,
    ...
    // World metadata for saving
    world_name: String,
    world_seed: u64,
    world_created_at: u64,  // Unix timestamp
    play_time_start: std::time::Instant,  // For tracking session time
    total_play_time: u64,  // Total seconds played
}

impl State {
    pub async fn new(window: Arc<Window>, world_size: WorldSize, instance: wgpu::Instance, surface: wgpu::Surface<'static>, world_name: String) -> Self {
        ...
        println!("Generated random seed: {}", world_seed);
        ...
        let world_created_at = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
            .as_secs();
        ...
        let spawn_x = (world_size.get_chunks() * CHUNK_WIDTH as i32 / 2) as i32;
        let spawn_z = (world_size.get_chunks() * CHUNK_WIDTH as i32 / 2) as i32;
        ...
        Self {
            ...
            world_seed,
            world_created_at,
            play_time_start: std::time::Instant::now(),
            total_play_time: 0,
        }
    }
    ...
    fn regenerate_chunk_at_position(&mut self, world_x: i32, world_z: i32) {
        ...
        let chunk_z = world_z.div_euclid(CHUNK_WIDTH as i32);
    ...
    /// Convert current game state to a saveable format
    fn to_world_save(&self) -> save::WorldSave {
        ...
        let session_time = self.play_time_start.elapsed().as_secs();
        let total_time = self.total_play_time + session_time;

        // Convert chunks
        let saved_chunks: Vec<save::SavedChunk> = self.world.get_all_chunks()
            .iter()
            .map(|(offset_x, offset_z, blocks)| {
                save::SavedChunk::from_chunk(*offset_x, *offset_z, blocks)
            })
            .collect();

        // Convert player state
        let player = save::SavedPlayerState {
            position: [self.camera.position.x, self.camera.position.y, self.camera.position.z],
            yaw: self.camera.yaw,
            pitch: self.camera.pitch,
            velocity: [
                self.player_state.velocity.x,
                self.player_state.velocity.y,
                self.player_state.velocity.z,
            ],
            on_ground: self.player_state.on_ground,
            is_flying: self.player_state.is_flying,
            physics_enabled: self.player_state.physics_enabled,
            game_mode: match self.game_mode {
                GameMode::Creative => save::SavedGameMode::Creative,
                GameMode::Spectator => save::SavedGameMode::Spectator,
            },
            selected_slot: self.hotbar.selected_slot,
            hotbar_blocks: self.hotbar.blocks.map(|b| b.id),
        };

        save::WorldSave {
            world_name: self.world_name.clone(),
            version: 1,
            created_at: self.world_created_at,
            last_played: now,
            play_time_seconds: total_time,
            seed: self.world_seed,
            world_size_chunks: self.world.get_size_chunks(),
            player,
            chunks: saved_chunks,
        }
    }

    /// Load game state from a saved world
    pub async fn from_world_save(
        window: Arc<Window>,
        world_save: save::WorldSave,
        instance: wgpu::Instance,
        surface: wgpu::Surface<'static>,
    ) -> Self {
        let size = window.inner_size();

        // Load game data from embedded JSON (same as new())
        let blocks_data: models::BlocksData = serde_json::from_str(BLOCKS_JSON)
            .expect("Failed to parse blocks.json");
        let items_data: models::ItemsData = serde_json::from_str(ITEMS_JSON)
            .expect("Failed to parse items.json");
        ...
        // Request GPU adapter (same as new())
        let adapter = instance.request_adapter(&wgpu::RequestAdapterOptions {
            power_preference: wgpu::PowerPreference::default(),
            compatible_surface: Some(&surface),
            force_fallback_adapter: false,
        ...
        // Setup texture bind groups (same as new())
        let texture_count = texture_map.len().max(1) as u32;
        let texture_bind_group_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            entries: &[
                wgpu::BindGroupLayoutEntry {
        ...
        // Restore camera from save
        let mut camera = Camera::new(size.width as f32 / size.height as f32, world_save.world_size_chunks);
        camera.position = glam::Vec3::new(
            world_save.player.position[0],
            world_save.player.position[1],
            world_save.player.position[2],
        );
        camera.yaw = world_save.player.yaw;
        camera.pitch = world_save.player.pitch;

        // Setup camera buffer (same as new())
        let camera_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("Camera Buffer"),
            contents: bytemuck::cast_slice(&[camera.get_view_projection()]),
            usage: wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST,
        ...
        // Load shaders and pipelines (same as new())
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("Shader"),
            source: wgpu::ShaderSource::Wgsl(include_str!("shader.wgsl").into()),
        });
        ...
        // Create render pipelines (opaque and transparent) - same as new()
        let render_pipeline_opaque = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("Opaque Pipeline"),
            layout: Some(&render_pipeline_layout),
            vertex: wgpu::VertexState {
        ...
        let render_pipeline_transparent = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            ...
            primitive: wgpu::PrimitiveState {
                ...
                strip_index_format: None,
                front_face: wgpu::FrontFace::Ccw,
                cull_mode: None,  // No culling for transparent blocks
                polygon_mode: wgpu::PolygonMode::Fill,
                unclipped_depth: false,
                conservative: false,
            },
            depth_stencil: Some(wgpu::DepthStencilState {
                format: wgpu::TextureFormat::Depth32Float,
                depth_write_enabled: false,  // Transparent blocks don't write depth
                depth_compare: wgpu::CompareFunction::Less,
                stencil: wgpu::StencilState::default(),
                bias: wgpu::DepthBiasState::default(),
            }),
        ...
        // Load world from save data
        let world = World::from_saved_chunks(
            world_save.chunks,
            world_save.world_size_chunks,
            block_registry.clone(),
        );

        println!("Loaded world with {} chunks", world.chunks.len());

        // Build chunk meshes from loaded world
        let chunk_meshes = world.chunks.iter().map(|((chunk_x, chunk_z), chunk)| {
            let (opaque_vertices, opaque_indices, transparent_vertices, transparent_indices) =
                chunk.generate_mesh(&world);
            ...
            ChunkMesh {
                opaque_vertex_buffer,
                opaque_index_buffer,
                opaque_num_indices: opaque_indices.len() as u32,
                transparent_vertex_buffer,
                transparent_index_buffer,
                transparent_num_indices: transparent_indices.len() as u32,
                chunk_x: *chunk_x,
                chunk_z: *chunk_z,
            }
        }).collect();
        ...
        let player_state = PlayerState {
            velocity: glam::Vec3::new(
                world_save.player.velocity[0],
                world_save.player.velocity[1],
                world_save.player.velocity[2],
            ),
            on_ground: world_save.player.on_ground,
            is_flying: world_save.player.is_flying,
            physics_enabled: world_save.player.physics_enabled,
        };

        let game_mode = match world_save.player.game_mode {
            save::SavedGameMode::Creative => GameMode::Creative,
            save::SavedGameMode::Spectator => GameMode::Spectator,
        };

        // Restore hotbar
        let mut hotbar = ui::Hotbar::new(&block_registry);
        hotbar.selected_slot = world_save.player.selected_slot;
        for i in 0..9 {
            hotbar.blocks[i] = block::BlockType::new(world_save.player.hotbar_blocks[i]);
        }
        ...
        Self {
            ...
            hotbar,
            texture_map,
            world_name: world_save.world_name,
            world_seed: world_save.seed,
            world_created_at: world_save.created_at,
            play_time_start: std::time::Instant::now(),
            total_play_time: world_save.play_time_seconds,
        }
    }

    /// Save the current world
    pub fn save_world(&self, compress: bool) -> Result<(), std::io::Error> {
        let world_save = self.to_world_save();
        save::save_world(&world_save, compress)
    }
